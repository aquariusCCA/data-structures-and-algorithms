...
前面我們已經完成了 AVL 樹的新增操作，接下來要實作的是**刪除操作**。

和新增一樣，刪除也可能改變樹的結構，因此刪除之後也要做兩件事：

1. **更新節點高度**
2. **檢查是否失衡，必要時重新平衡**

這也是 AVL 樹刪除和普通二叉搜索樹刪除最大的差別。

## 一、先定義對外的刪除方法

我們先定義一個對外的方法 `remove`，接收要刪除的 `key`。

這裡為了簡化，我們不返回被刪掉的值，直接設計成 `void` 即可。
...
這裡一定要把返回值重新賦值給 `root`，因為：

* 刪除後，根節點可能改變
* 刪除後如果發生旋轉，根節點也可能改變

所以不能只是呼叫 `doRemove`，還必須接住它返回的新根。
...
如果目前節點已經是 `null`，表示這棵子樹本來就空，或者一路找下去也沒有找到要刪除的 key。

這時就不需要再做任何事情，直接返回 `null` 即可。
...
## 五、情況二：還沒找到 key，繼續往下找

如果目前節點不是 `null`，但還沒找到要刪除的 key，就按照二叉搜索樹的規則繼續查找：
...
不能只是單純呼叫遞迴，因為 `doRemove` 返回的是**刪除後剩下的新子樹根節點**，必須重新接回目前節點的左孩子或右孩子。
...
但找到之後，不能只用一種方式處理，因為被刪除節點可能有四種不同情況：
...
如果被刪除節點左孩子為空，但右孩子不為空，表示刪掉目前節點後，剩下來的就是它的右孩子。
...
但在 AVL 樹中，我們先不要急著立刻返回，因為後面還要做：

* 更新高度
* 平衡檢查

所以我們改成先把「刪剩下的節點」存回 `node`：
...
如果右孩子為空，但左孩子不為空，表示刪掉目前節點後，剩下來的是左孩子。
...
同樣先不要立刻 `return`，而是讓程式繼續往下執行，去更新高度並重新平衡。

### 刪除情況四：左右孩子都有

這是刪除中最複雜的一種情況。

當被刪除節點同時有左孩子和右孩子時，不能直接把它刪掉，否則整棵樹的結構會斷掉。
...
因為後繼節點本身可能還帶著自己的後代節點。

例如剛才刪除 `9` 的例子中，後繼節點 `10` 還有右孩子 `11`。

所以不能只是把 `10` 拿過來放到 `9` 的位置就結束，還必須先處理 `10` 原本所在位置的後代。
...
這棵刪除後剩下的右子樹，就要接到 `s.right`。

#### 5. 再接上左子樹

被刪除節點原本的左子樹，也要接回後繼節點：

```java
s.left = node.left;
```

這樣後繼節點才能完整接手原本被刪除節點的位置。
...
先處理右子樹，把後繼節點原本所在位置整理乾淨，再接回左子樹，整體邏輯會比較清楚，也比較不容易把關係弄亂。
...
AVL 樹的刪除，本質上還是二叉搜索樹的刪除，只是比普通 BST 多了最後兩步：
...
因為 `9` 有左右兩個孩子，所以進入「左右孩子都有」的情況。

### 第二步：找後繼節點

`9` 的右子樹是 `12`，一路往左找，找到 `10`。
...
* `10.left` 接到原本 `9.left`
* `10.right` 接到刪除後剩下的右子樹

這樣 `10` 就正式取代 `9`。

### 第五步：更新高度並檢查平衡

這時新的子樹根節點是 `10`，接著要更新它的高度，再檢查是否失衡。

根據筆記中的分析，這時會出現失衡，而且屬於 **LL** 情況，所以還要再做一次右旋。

旋轉後，左子樹會重新恢復平衡。

## 十一、為什麼刪除也和新增一樣，是「沿著回去的路」調整？

這一點非常重要。

無論是新增還是刪除，AVL 都不是一次重新計算整棵樹，而是：
