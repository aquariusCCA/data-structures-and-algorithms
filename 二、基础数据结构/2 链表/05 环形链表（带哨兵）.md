...
> 关键字：环形链表、循环链表、哨兵节点、尾节点、next、prev  
> 建議回查情境：想回查循環鏈表的首尾連接方式，或比較它和普通鏈表的差異時

## 本节导读

這一節整理帶哨兵的環形鏈表，重點是尾部不再指向 `null`，而是回到哨兵或頭部形成閉環。第一次閱讀時建議先理解初始化時的自引用，再看新增與刪除如何保持環狀結構。
...
- 上一篇：[双向链表（带哨兵）](./04%20双向链表（带哨兵）.md)
- 返回：[链表入口](./README.md)
- 下一篇：[LeetCode 206 反转链表](./06%20LeetCode%20206%20反转链表.md)
