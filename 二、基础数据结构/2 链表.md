...
構造一個新鏈表，從 **舊鏈表** 依次拿到每個節點，創建新節點添加至 **新鏈表** 頭部，完成後新鏈表即是倒序的
...
與方法一類似，構造一個新鏈表，從 **舊鏈表頭部** 移除節點，添加到 **新鏈表頭部**，完成後新鏈表即是倒序的，區別在於原題目未提供節點外層的容器類，這裡提供一個，另外一個區別是並不去構造新節點。
...
遞歸，利用 **歸** 時倒序的特性，完成讓 $5 \mapsto 4, 4 \mapsto 3 ...$

首先，寫一個遞歸方法，返回值用來拿到最後一個節點
...
* 注意1：遞歸終止條件是 `curr.next == null`，目的是到最後一個節點就結束遞歸，與之前遞歸遍歷不一樣
* 注意2：需要考慮空鏈表即 `p == null` 的情況
...
下面 **偽代碼** 調用過程，假設節點分別是 $1 \mapsto 2 \mapsto 3 \mapsto 4 \mapsto 5 \mapsto null$，先忽略返回值

```java
public ListNode reverseList(ListNode p = 1) {
    public ListNode reverseList(ListNode p = 2) {
        public ListNode reverseList(ListNode p = 3) {
            public ListNode reverseList(ListNode p = 4) {
                public ListNode reverseList(ListNode p = 5) {
...
接下來，從 `p = 4` 開始，要讓 $5 \mapsto 4, 4 \mapsto 3 ...$
    ...
    public ListNode reverseList(ListNode p = 2) {
        public ListNode reverseList(ListNode p = 3) {
            public ListNode reverseList(ListNode p = 4) {
                ...
                // 此時 p 是 4，p.next 是 5，要讓 5 指向 4，代碼寫成 p.next.next = p
                // 還要注意 4 要指向 null，否則就死循環了
            }
            // 此時 p 是 3，p.next 是 4
        }
        // 此時 p 是 2，p.next 是 3
...
> 評價：單向鏈表沒有 `prev` 指針，但利用遞歸的特性 **記住了** 鏈表倒過來的順序

## 方法四

從鏈表每次拿到第二個節點，將其從鏈表斷開，插入頭部，直至它為 null 結束

1. 設置指針 o1（舊頭）、n1（新頭）、o2（舊老二），分別指向第一、第一、第二節點
   - $\frac{{n1} {o1}}{1} \mapsto \frac{o2}{2} \mapsto 3 \mapsto 4 \mapsto 5 \mapsto null$
2. 將 o2 節點從鏈表斷開，即 o1 節點指向第三節點
   - $\frac{{n1} {o1}}{1} \mapsto 3 \mapsto 4 \mapsto 5 \mapsto null，\frac{o2}{2}$
3. o2 節點鏈入鏈表頭部，即
   - $\frac{o2}{2} \mapsto \frac{{n1} {o1}}{1} \mapsto 3 \mapsto 4 \mapsto 5 \mapsto null$
4. n1 指向 o2
   - $\frac{{n1} {o2}}{2} \mapsto \frac{o1}{1} \mapsto 3 \mapsto 4 \mapsto 5 \mapsto null$
5. o2 指向 o1 的下一個節點，即
   - $\frac{n1}{2} \mapsto \frac{o1}{1} \mapsto \frac{o2}{3} \mapsto 4 \mapsto 5 \mapsto null$
6. 重複以上 $2 ~ 5$ 步，直到 o2 指向 null
7. 還應當考慮邊界條件，即鏈表中不滿兩個元素時，無需走以上邏輯
...
要點：把鏈表分成兩部分，思路就是不斷從鏈表 2 的頭，網戀表 1 的頭搬移

1. n1 指向 null，代表 **新鏈表** 一開始沒有元素，o1 指向 **原鏈表** 的首節點
   - $\frac{n1}{null} ， \frac{o1}{1} \mapsto 2 \mapsto 3 \mapsto 4 \mapsto 5 \mapsto null$
2. 開始循環，o2 指向 **原鏈表** 次節點
   - $\frac{n1}{null} ， \frac{o1}{1} \mapsto \frac{o2}{2} \mapsto 3 \mapsto 4 \mapsto 5 \mapsto null$
3. 搬移
   - $\frac{o1}{1} \mapsto \frac{n1}{null} ， \frac{o2}{2} \mapsto 3 \mapsto 4 \mapsto 5 \mapsto null$
4. 指針復位
   - $\frac{n1}{1} \mapsto null， \frac{o1 o2}{2} \mapsto 3 \mapsto 4 \mapsto 5 \mapsto null$
5. 重複 $2 ~ 4$ 步
6. 當 `o1 = null` 時退出循環
